...
function CommissionModal({ source, plan, onClose }: { source: AnyRecord; plan?: AnyRecord; onClose: () => void }) {
  ...
  return <div className="modal-backdrop"><section className="decision-modal obligation-modal commission-v4-modal" role="dialog" aria-modal="true">
    ...
    <div className="modal-actions"><button className="secondary-button" type="button" onClick={onClose}>Fechar</button>{canOperate && editable && <button className="revenue-button" type="button" disabled={busy} onClick={() => void saveSchedule()}><Send size={16} /> Enviar líquido para aprovação</button>}{canApprove && plan?.status === 'aguardando_aprovacao' && <><button className="expense-button" type="button" disabled={busy} onClick={() => void decide('rejeitado')}>Rejeitar</button><button className="revenue-button" type="button" disabled={busy} onClick={() => void decide('aprovado')}><CheckCircle2 size={16} /> Aprovar comissão</button></>}{canOperate && plan && ['aprovado', 'parcialmente_pago'].includes(String(plan.status)) && <button className="revenue-button" type="button" disabled={busy} onClick={() => void saveExecution()}><CheckCircle2 size={16} /> Salvar pagamentos</button>}</div>
  </section></div>
}
...
export function AgentCommissionsPageV4() {
  ...
  const sources = useMemo(() => receivables.filter((item) => eligible(item) && `${item.processo ?? ''} ${item.reclamante ?? ''} ${item.reclamada ?? ''} ${beneficiary(item)}`.toLowerCase().includes(search.toLowerCase())), [receivables, search])
