...
function ExpenseModal({ record, onClose }: { record?: AnyRecord | null; onClose: () => void }) {
  ...
  return <div className="modal-backdrop"><section className="modal-sheet legacy-sheet expense-sheet" role="dialog" aria-modal="true">
    <div className="modal-toolbar"><div><span className="eyebrow expense-text">Tesouraria</span><h2>{modalTitle}</h2></div><button className="icon-button" type="button" onClick={onClose}><X size={20} /></button></div>
    ...
    <div className="modal-actions"><button className="secondary-button" type="button" onClick={onClose}>Cancelar</button>{isDraft ? <><button className="outline-expense-button" type="button" disabled={busy} onClick={() => save('rascunho')}>Salvar alterações</button><button className="expense-button" type="button" disabled={busy} onClick={() => save('enviado_aprovacao')}><Send size={17} /> {busy ? 'Enviando...' : 'Enviar para Aprovação'}</button></> : isReturned ? <><button className="outline-expense-button" type="button" disabled={busy} onClick={() => save('devolvido')}>Salvar correção</button><button className="expense-button" type="button" disabled={busy} onClick={() => save('enviado_aprovacao')}><Send size={17} /> {busy ? 'Enviando...' : 'Reenviar para Aprovação'}</button></> : <><button className="outline-expense-button" type="button" disabled={busy} onClick={() => save('rascunho')}>Salvar rascunho</button><button className="expense-button" type="button" disabled={busy} onClick={() => save('enviado_aprovacao')}><Send size={17} /> {busy ? 'Enviando...' : 'Enviar para Aprovação'}</button></>}</div>
  </section></div>
}
...
export function ExpensesPageStorage() {
  ...
  const canOperate = ['master', 'gerente', 'tesouraria', 'operador'].includes(String(profile?.role ?? ''))
