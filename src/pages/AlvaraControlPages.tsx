...
function componentValue(item: AnyRecord, name: string) {
  ...
  const found = components.find((component) => String(component?.nome ?? '').toLowerCase() === name.toLowerCase())
  return toNumber(found?.valor)
}

function eligibleReceivable(item: AnyRecord) {
  return ['enviado_tesouraria', 'recebido_tesouraria', 'encerrado'].includes(String(item.status ?? ''))
}
...
function sourceAmount(source: AnyRecord, kind: PaymentKind) {
  return kind === 'client'
    ? toNumber(source.valorLiquidoCliente)
    : toNumber(source.agentCommissionValue) || componentValue(source, 'Outras Deduções / Participações')
}

function sourceBeneficiary(source: AnyRecord, kind: PaymentKind) {
  return kind === 'client'
    ? String(source.titular || source.reclamante || 'Cliente')
    : String(source.agentName || 'Agente não informado')
}

function planPaidValue(plan?: AnyRecord) {
  const installments = Array.isArray(plan?.installments) ? plan.installments : []
  return installments.reduce((sum: number, item: DocumentData) => sum + (item?.paid ? toNumber(item.value) : 0), 0)
}
...
function statusClass(plan?: AnyRecord) {
  ...
  if (plan.status === 'pago') return 'workflow-success'
  ...
  if (plan.status === 'aprovado') return 'workflow-success'
...
function SourceSnapshot({ source, kind }: { source: AnyRecord; kind: PaymentKind }) {
  return <div className="obligation-source-grid">
    ...
    {kind === 'client' && <>
      <div><span>Banco</span><strong>{source.banco || '—'}</strong></div>
      <div><span>Agência / Conta</span><strong>{source.agencia || '—'} / {source.conta || '—'}</strong></div>
      <div><span>CPF</span><strong>{source.cpf || '—'}</strong></div>
    </>}
    <div className="obligation-source-value"><span>Valor devido</span><strong>{money.format(sourceAmount(source, kind))}</strong></div>
  </div>
}
...
function PaymentPlanModal({ source, plan, kind, collectionName, onClose }: { source: AnyRecord; plan?: AnyRecord; kind: PaymentKind; collectionName: string; onClose: () => void }) {
  ...
  const [installments, setInstallments] = useState<Installment[]>(() => {
    const existing = Array.isArray(plan?.installments) ? plan!.installments as Installment[] : []
  ...
  async function saveSchedule() {
    ...
    if (!installments.length || installments.some((item) => !item.dueDate || toNumber(item.value) <= 0)) {
      window.alert('Informe valor e data de todas as parcelas.')
      return
    }
    if (Math.abs(sum - amount) > 0.02) {
      window.alert(`A soma das parcelas deve ser igual ao valor devido (${money.format(amount)}).`)
      return
    }
    setBusy(true)
    try {
      const payload = {
        ...
        reclamante: source.reclamante || '',
        reclamada: source.reclamada || '',
        beneficiary: sourceBeneficiary(source, kind),
        amountDue: amount,
        paymentType,
        installmentCount: installments.length,
        firstDueDate: installments[0]?.dueDate || firstDueDate,
        installments: installments.map((item, index) => ({ ...item, number: index + 1, paid: false, paidDate: '' })),
        ...
        approvedAt: null,
        sourceSnapshot: {
          unidade: source.unidade || '', data: source.data || '', natureza: source.natureza || '', processo: source.processo || '',
          reclamante: source.reclamante || '', reclamada: source.reclamada || '', origem: source.origem || '',
          banco: source.banco || '', agencia: source.agencia || '', conta: source.conta || '', titular: source.titular || '', cpf: source.cpf || '',
          agentName: source.agentName || '', valorLiquidoCliente: toNumber(source.valorLiquidoCliente), agentCommissionValue: sourceAmount(source, 'agent'),
        },
        createdBy: plan?.createdBy || profile.uid,
        createdByName: plan?.createdByName || profile.displayName,
        createdAt: plan?.createdAt || serverTimestamp(),
        updatedBy: profile.uid,
        updatedByName: profile.displayName,
        updatedAt: serverTimestamp(),
      }
      await setDoc(doc(db, collectionName, source.id), payload, { merge: false })
      ...
      console.error(error)
      window.alert('Não foi possível salvar a programação.')
  ...
  async function decide(status: 'aprovado' | 'rejeitado') {
    ...
    try {
      ...
      console.error(error)
      window.alert('Não foi possível registrar a decisão.')
    } finally { setBusy(false) }
  }
  ...
  async function saveExecution() {
    ...
    try {
      await setDoc(doc(db, collectionName, source.id), {
        installments,
        status: nextStatus,
        paidValue: installments.reduce((total, item) => total + (item.paid ? toNumber(item.value) : 0), 0),
        updatedBy: profile.uid,
        updatedByName: profile.displayName,
        updatedAt: serverTimestamp(),
      }, { merge: true })
      ...
      console.error(error)
      window.alert('Não foi possível atualizar a execução do pagamento.')
    } finally { setBusy(false) }
  }
  ...
  return <div className="modal-backdrop"><section className="decision-modal obligation-modal" role="dialog" aria-modal="true">
    ...
    {!isExecutionStage && <>
      <div className="obligation-form-grid">
        <label><span>Forma de pagamento</span><select value={paymentType} disabled={!canOperate || plan?.status === 'aguardando_aprovacao'} onChange={(event) => { const value = event.target.value as 'avista' | 'parcelado'; setPaymentType(value); if (value === 'avista') setCount(1) }}><option value="avista">À vista</option><option value="parcelado">Parcelado</option></select></label>
        <label><span>Número de parcelas</span><input type="number" min="1" max="60" value={paymentType === 'avista' ? 1 : count} disabled={!canOperate || paymentType === 'avista' || plan?.status === 'aguardando_aprovacao'} onChange={(event) => setCount(Math.max(1, Number(event.target.value)))} /></label>
        <label><span>Primeira data prevista</span><input type="date" value={firstDueDate} disabled={!canOperate || plan?.status === 'aguardando_aprovacao'} onChange={(event) => setFirstDueDate(event.target.value)} /></label>
      </div>
      {canOperate && plan?.status !== 'aguardando_aprovacao' && <button className="secondary-button obligation-generate" type="button" onClick={regenerate}>Gerar / atualizar parcelas</button>}
    </>}

    <div className="obligation-installments">
      <div className="obligation-installment-head"><span>Parcela</span><span>Valor</span><span>Data prevista</span>{isExecutionStage && <><span>Pago</span><span>Data do pagamento</span></>}</div>
      {installments.map((item, index) => <div className="obligation-installment-row" key={item.number}>
        <strong>{index + 1}/{installments.length}</strong>
        <input type="number" min="0" step="0.01" value={item.value} disabled={isExecutionStage || !canOperate || plan?.status === 'aguardando_aprovacao'} onChange={(event) => patchInstallment(index, { value: Number(event.target.value) })} />
        <input type="date" value={item.dueDate} disabled={isExecutionStage || !canOperate || plan?.status === 'aguardando_aprovacao'} onChange={(event) => patchInstallment(index, { dueDate: event.target.value })} />
        {isExecutionStage && <><label className="obligation-paid-check"><input type="checkbox" checked={Boolean(item.paid)} disabled={!canOperate || plan?.status === 'pago'} onChange={(event) => patchInstallment(index, { paid: event.target.checked, paidDate: event.target.checked ? (item.paidDate || today()) : '' })} /><span>{item.paid ? 'Paga' : 'Pendente'}</span></label><input type="date" value={item.paidDate || ''} disabled={!canOperate || !item.paid || plan?.status === 'pago'} onChange={(event) => patchInstallment(index, { paidDate: event.target.value })} /></>}
      </div>)}
      <div className="obligation-installment-total"><span>Total programado</span><strong className={Math.abs(sum - amount) > .02 ? 'expense-text' : ''}>{money.format(sum)}</strong></div>
    </div>
    ...
    <div className="modal-actions">
      ...
      {canOperate && (!plan || plan.status === 'rejeitado') && <button className="revenue-button" type="button" disabled={busy} onClick={() => void saveSchedule()}><Send size={16} /> Enviar para aprovação</button>}
      {canApprove && plan?.status === 'aguardando_aprovacao' && <><button className="expense-button" type="button" disabled={busy} onClick={() => void decide('rejeitado')}>Rejeitar</button><button className="revenue-button" type="button" disabled={busy} onClick={() => void decide('aprovado')}><CheckCircle2 size={16} /> Aprovar pagamento</button></>}
      {canOperate && plan && ['aprovado', 'parcialmente_pago'].includes(String(plan.status)) && <button className="revenue-button" type="button" disabled={busy} onClick={() => void saveExecution()}><CheckCircle2 size={16} /> Salvar pagamentos</button>}
    </div>
  </section></div>
}
...
function PaymentControlPage({ kind }: { kind: PaymentKind }) {
  ...
  const subtitle = kind === 'client'
    ? 'Controle separado do valor devido ao cliente, independentemente do mês em que o Alvará entrou no caixa.'
    : 'Controle dos pagamentos de comissões e participações de agentes, inclusive quando realizados em parcelas ou meses diferentes.'
  ...
  const sources = useMemo(() => receivables.filter((item) => {
    ...
    const haystack = `${item.processo ?? ''} ${item.reclamante ?? ''} ${item.reclamada ?? ''} ${sourceBeneficiary(item, kind)}`.toLowerCase()
  ...
  const totals = useMemo(() => sources.reduce((acc, source) => {
    ...
    const installments = Array.isArray(plan?.installments) ? plan!.installments : []
    acc.overdue += installments.filter((item: DocumentData) => !item.paid && item.dueDate && String(item.dueDate) < today() && ['aprovado', 'parcialmente_pago'].includes(String(plan?.status))).length
    return acc
  }, { due: 0, paid: 0, pending: 0, approvals: 0, overdue: 0 }), [kind, planMap, sources])

  const loading = loadingReceivables || loadingPlans
  const Icon = kind === 'client' ? BadgeDollarSign : Users

  return <>
    <div className="page-heading"><div><span className="eyebrow">Controle financeiro separado</span><h1>{title}</h1><p>{subtitle}</p></div></div>
    ...
    <section className="page-card module-card"><div className="module-toolbar"><div className="search-box"><Search size={17} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por processo, cliente, parte ou beneficiário" /></div></div>
      {loading ? <div className="module-empty"><RefreshCw className="spin" size={30} /><strong>Carregando controle</strong></div> : sources.length === 0 ? <div className="module-empty"><Icon size={34} /><strong>Nenhum valor disponível para este controle</strong><span>Os registros são alimentados automaticamente pelo Demonstrativo de Recebimento de Honorários.</span></div> : <div className="obligation-list">
        <div className="obligation-list-row obligation-list-head"><span>Processo / Beneficiário</span><span>Valor devido</span><span>Já pago</span><span>Saldo</span><span>Status</span><span>Ação</span></div>
        {sources.map((source) => {
          const plan = planMap.get(source.id)
          const due = sourceAmount(source, kind)
          const paid = planPaidValue(plan)
          return <div className="obligation-list-row" key={source.id}>
            <span><strong>{source.processo || '—'}</strong><small>{sourceBeneficiary(source, kind)}</small></span>
            <span><strong>{money.format(due)}</strong></span>
            <span><strong>{money.format(paid)}</strong></span>
            <span><strong>{money.format(Math.max(0, due - paid))}</strong></span>
            <span><b className={`workflow-status-badge ${statusClass(plan)}`}>{statusLabel(plan)}</b></span>
            <span><button className="small-neutral-button" type="button" onClick={() => setTarget(source)}>{plan ? 'Abrir controle' : 'Programar pagamento'}</button></span>
          </div>
        })}
      </div>}
    </section>
...
function FiscalNoteModal({ source, note, onClose }: { source: AnyRecord; note?: AnyRecord; onClose: () => void }) {
  ...
  async function save() {
    ...
    if (status === 'emitida' && (!number.trim() || !issueDate)) {
      window.alert('Para marcar como Emitida, informe número e data da Nota Fiscal.')
      return
    }
    setBusy(true)
    try {
      await setDoc(doc(db, 'fiscalNotes', source.id), {
        ...
        notes: notes.trim(),
        sourceSnapshot: {
          unidade: source.unidade || '', natureza: source.natureza || '', processo: source.processo || '', reclamante: source.reclamante || '', reclamada: source.reclamada || '',
          titular: source.titular || '', cpf: source.cpf || '', emailNf: source.emailNf || '', enderecoNf: source.enderecoNf || '', valorAlvara: toNumber(source.valorAlvara), invoiceValue,
        },
        updatedBy: profile.uid, updatedByName: profile.displayName, updatedAt: serverTimestamp(),
        createdBy: note?.createdBy || profile.uid, createdAt: note?.createdAt || serverTimestamp(),
      }, { merge: false })
      ...
      console.error(error)
      window.alert('Não foi possível atualizar o controle da Nota Fiscal.')
    } finally { setBusy(false) }
  }

  return <div className="modal-backdrop"><section className="decision-modal obligation-modal" role="dialog" aria-modal="true">
    <div className="modal-toolbar"><div><span className="eyebrow">Nota Fiscal</span><h2>Dados para emissão</h2></div><button className="icon-button" type="button" onClick={onClose}><X size={20} /></button></div>
    <div className="fiscal-client-card">
      <div><span>Cliente</span><strong>{source.titular || source.reclamante || '—'}</strong></div><div><span>CPF</span><strong>{source.cpf || '—'}</strong></div>
      <div><span>E-mail</span><strong>{source.emailNf || '—'}</strong></div><div><span>Endereço</span><strong>{source.enderecoNf || '—'}</strong></div>
      <div><span>Processo</span><strong>{source.processo || '—'}</strong></div><div className="fiscal-value"><span>Valor da Nota / Honorários</span><strong>{money.format(invoiceValue)}</strong></div>
    </div>
    <div className="obligation-form-grid fiscal-form-grid"><label><span>Status</span><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="pendente">Pendente de emissão</option><option value="emitida">Emitida</option><option value="cancelada">Cancelada</option></select></label><label><span>Número da Nota Fiscal</span><input value={number} onChange={(event) => setNumber(event.target.value)} /></label><label><span>Data de emissão</span><input type="date" value={issueDate} onChange={(event) => setIssueDate(event.target.value)} /></label></div>
    ...
    <div className="modal-actions"><button className="secondary-button" type="button" onClick={onClose}>Fechar</button>{canEdit && <button className="revenue-button" type="button" disabled={busy} onClick={() => void save()}><FileText size={16} /> Salvar controle da NF</button>}</div>
  </section></div>
}
...
export function FiscalNotesPage() {
  ...
  const sources = useMemo(() => receivables.filter((item) => {
    if (!eligibleReceivable(item)) return false
    const value = toNumber(item.invoiceValue) || componentValue(item, 'Honorários do Escritório')
    if (value <= 0) return false
    return `${item.processo ?? ''} ${item.reclamante ?? ''} ${item.titular ?? ''} ${item.cpf ?? ''}`.toLowerCase().includes(search.toLowerCase())
  }), [receivables, search])
  ...
  return <>
    <div className="page-heading"><div><span className="eyebrow">Apoio à emissão</span><h1>Nota Fiscal</h1><p>Os dados do cliente e o valor dos honorários são trazidos automaticamente do Demonstrativo de Recebimento de Honorários.</p></div></div>
